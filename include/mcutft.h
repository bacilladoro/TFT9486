#ifndef mcutft_H_
#define mcutft_H_   299
#include <Arduino.h>
#include <SPI.h>
#include <./Fruit_GFX.h>

class mcutft : public Fruit_GFX {

	public:
	mcutft(int CS=A3, int RS=A2, int WR=A1, int RD=A0, int RST=A4); //shield wiring
	//mcutft(int CS=0, int RS=0, int WR=0, int RD=0, int _RST=0);  //dummy arguments 
	void     reset(void);                                       // you only need the constructor
	void     begin(uint16_t ID = 0x9486);                       // you only need the constructor
